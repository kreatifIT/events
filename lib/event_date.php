...
class event_date extends \rex_yform_manager_dataset
{
    ...
    {
        $this->category = $this->getRelatedDataset('categories');
    ...
    public function getIcs()
    {
        ...
        // date/time
        $vEvent->setUseTimezone(true)->setDtStart($this->getStartDate())->setDtEnd($this->getEndDate())->setCreated(new \DateTime($this->getValue("createdate")))->setModified(new \DateTime($this->getValue("updatedate")))->setNoTime($this->getValue("all_day"))
            // ->setNoTime($is_fulltime) // Wenn Ganztag
            // ->setCategories(explode(",", $sked['entry']->category_name))
            ->setSummary($this->getName())->setDescription($this->getDescriptionAsPlaintext());
